...
function getProdId(prodid){
	//window.opener.document.addComp.technician_id.value = techid;
	window.opener.document.getElementById('product_id').value = prodid;
	//alert('success');
	window.close();
}
</script>
         	...
         	<c:forEach begin="1" end="${noOfPages}" var="i">
         		<c:choose>
	                <c:when test="${currentPage eq i}">
	                	...
	                	<c:if test="${i > 3}">
	                		<li class="gen"><a href="ProductDetails?page=${i+3}&q=${searchtext}">${i+3}</a></li>
	                	</c:if>
	                	<c:if test="${i > 4}">
	                		<li class="gen"><a href="ProductDetails?page=${i+4}&q=${searchtext}">${i+4}</a></li>
	                	</c:if>
	                </c:when>
	         		<c:otherwise>
	                     <!--  <li class="gen"><a href="ProductDetails?page=${i}&q=${searchtext}">${i}</a></li>-->
	                </c:otherwise>
            	</c:choose>
         	</c:forEach>
         </li>
    ...
    <div class="table-responsive ">
      <table id="result_list" class="table table-hover">
        ...
        <th scope="col" class="warning gen"><input type="checkbox" id="selectall">
          ...
          <th scope="col" class="warning gen"><a href="ProductDetails?q=${searchtext}&o=1">Name</a>
            <div class="clear"></div>
          </th>
          <th scope="col" class="warning gen"><a href="ProductDetails?q=${searchtext}&o=2">Slug</a>
            <div class="clear"></div>
          </th>
          <th scope="col" class="warning gen"><a href="ProductDetails?q=${searchtext}&o=3">Is active</a>
            <div class="clear"></div>
          </th>
        ...
        <tbody>
          	<c:forEach var="Product" varStatus="loopStatus" items="${products}">
				<tr class="${loopStatus.index % 2 == 0 ? 'success' : 'row1'}">
					...
					<td><a class="gen" href="#" onClick="javascript:getProdId(${Product.id});"><c:out value="${Product.name}"></c:out></a></td>
