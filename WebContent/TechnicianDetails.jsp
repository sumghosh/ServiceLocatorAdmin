...
$(document).ready(function () {
    ...
    $('select').change(function(){
        ...
        if(opt != 'ALL'){
                var size = $('tr#techrow').length;
                for(var i=0; i<size; i++){
                		//alert($('tr#techrow:eq('+i+') td:eq(6)').text()+':::::'+opt);
                        if($('tr#techrow:eq('+i+') td:eq(6)').text() != opt){
                                $('tr#techrow:eq('+i+')').hide();
                        }else{
                                $('tr#techrow:eq('+i+')').show();
                        }
                }
        }else{
                $('tr#techrow').show();
...
<div class="col-sm-8 col-md-12 col-lg-12 col-xs-12 main rightnav">
  <div class="row col-sm-6 col-md-12 col-lg-12"> <span class="header_top" style="margin-left:20px;">Technician details</span>
  ...
  <div class="panel_search panel-default col-sm-4 col-md-11 col-lg-11" style="margin-left:20px;">
    <form id="technician-search" action="" method="get">
      ...
      <div class="form-group_search col-sm-6 col-md-2 col-lg-2">
        <a type="button" class="btn btn-success" onclick="document.getElementById('technician-search').submit();">SEARCH</a>
      </div>
        ...
        <div class="input-group" style="margin-right:10px;">
          ...
          <select id="countryoption" name="action" class="form-control">
            ...
            <c:forEach var="Technician" items="${technicians}">
          		<option value="${Technician.techAddress.country}" >
          			<c:out value="${Technician.techAddress.country}"></c:out>
          		</option>
          	</c:forEach>
          </select>
        </div>
      ...
      <table id="result_list" class="table table-hover" width="100%">
        ...
        <tbody>
          <c:forEach var="Technician" varStatus="loopStatus" items="${technicians}">
				<tr id="techrow" class="${loopStatus.index % 2 == 0 ? 'success' : 'row1'}">
					<td><input class="case" name="case" type="checkbox" value="" /></td>
					<td><a class="gen" href="EditTechnician?techid=${Technician.id}"><c:out value="${Technician.userId}"></c:out></a></td>
					...
					<tr><td>
						<form id="deltechnician" method="get" action="deleteTechnician">
 							...
 							<a class="glyphicon glyphicon-remove remove" href="deleteTechnician?techid=${Technician.id}" onclick="javascript:delete_technician();"></a>
						</form>
					</td>
					<td>
						<form id="edittechnician" method="get" action="EditTechnician">
							<input type="hidden" name="edit" value="${Technician.id}" />
							<a class="glyphicon glyphicon-pencil edit" href="EditTechnician?techid=${Technician.id}" onclick="document.getElementById('edittechnician').submit();"></a>
						</form></td>
						</tr>
					</table>
					</td>
