...
public class displayUsers extends HttpServlet {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
    	...
    	try {
        	...
        	if(request.getParameter("page") != null && request.getParameter("page") != "")
                ...
                List<User> result = qwc.displayUsers((page - 1)*Constants.RECORDS_PER_PAGE, Constants.RECORDS_PER_PAGE);
                ...
                view = request.getRequestDispatcher("UserDetails.jsp");
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
    	...
    	try{
        	...
        	if(request.getParameter("page") != null && request.getParameter("page") != "")
                page = Integer.parseInt(request.getParameter("page"));
            List<User> totalResult = qwc.displayUsers((page - 1)*Constants.RECORDS_PER_PAGE, Constants.RECORDS_PER_PAGE);
            request.setAttribute("users", totalResult);
...
//	    	            view.forward(request, response);
			    		...
			    		if (sort_column == 1)
			    			query = Constants.USERNAME;
			    		else if (sort_column == 2)
			    			query = Constants.EMAIL;
			    		else if (sort_column == 3)
			    			query = Constants.FIRSTNAME;
			    		else if (sort_column == 4)
			    			query = Constants.LASTNAME;
			    		else if (sort_column == 5)
			    			query = Constants.STAFFSTATUS;
			            List<User> result = qwc.sortUsers(query_param, query, (page - 1)*Constants.RECORDS_PER_PAGE, Constants.RECORDS_PER_PAGE);
			            request.setAttribute("users", result);
	            	...
	            	if (!request.getParameterMap().containsKey("o")){
			            List<User> searchResult = qwc.searchUsers(query_param, (page - 1)*Constants.RECORDS_PER_PAGE, Constants.RECORDS_PER_PAGE);
